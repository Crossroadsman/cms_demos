...
#
# 1. [x] Create the Django Project
# 2. [x] Create a `users` app
# 3. [x] Decide whether to subclass `AbstractUser` or `AbstractBaseUser`
#        (this project uses the former, which is much, much easier)
# 4. [x] Create initial custom user model:
#    4.1 [x] update `settings.py` (see `settings.py` for more details)
#    4.2 [x] create a replacement `User` model (see below)
#    4.3 [x] create new user forms (see `forms.py`)
#    4.4 [x] customise Django admin (see `admin.py`)
